...
impl Context {
    ...
    /// debug_layer0(embedding: [d]float32) -> dict[str, [d]float32
    ///
    /// Run layer 0 linear attention in CPU mode on a single embedding vector,
    /// capturing every intermediate tensor. Returns a dict with keys:
    ///   normed, qkv, z, beta, alpha, conv_out, lin_q, lin_k, lin_v,
    ///   q_normed, k_normed, out_values, gated_out, attn_out, hidden_out
    fn debug_layer0(&self, py: Python<'_>, embedding: &Bound<PyArray1<f32>>) -> PyResult<PyObject> {
        let m = self.model.as_ref().ok_or_else(||
            pyo3::exceptions::PyRuntimeError::new_err("No model loaded"))?;
        let emb = embedding.readonly();
        let emb_slice = emb.as_slice()?;
        let hd = m.config.hidden_dim;
        let total_key = m.config.linear_total_key;
        let total_value = m.config.linear_total_value;
        ...
        let qkv_dim = total_key * 2 + total_value;
        let prefix = "model.layers.0.linear_attn";

        let mut hidden = emb_slice.to_vec();
        let residual = hidden.clone();

        // 1. Input RMS norm
        let nw = m.wf.get_tensor_u16("model.layers.0.input_layernorm.weight")
            .ok_or_else(|| pyo3::exceptions::PyRuntimeError::new_err("missing input_layernorm"))?;
        let nw_f32: Vec<f32> = nw.iter().map(|&v| bf16_to_f32(v)).collect();
        if nw_f32.len() != hd {
            return Err(pyo3::exceptions::PyRuntimeError::new_err(
                format!("input_layernorm weight has {} elements, expected {}", nw_f32.len(), hd)
            ));
        }
        let mut normed = vec![0.0f32; hd];
        cpu_rms_norm(&hidden, &nw_f32, &mut normed, hd, RMS_NORM_EPS);

        // 2-5. Attention projections
        ...
        if let (Some(w), Some(s), Some(b)) = (
            m.wf.get_tensor_u32(&format!("{}.in_proj_qkv.weight", prefix)),
            m.wf.get_tensor_u16(&format!("{}.in_proj_qkv.scales", prefix)),
            m.wf.get_tensor_u16(&format!("{}.in_proj_qkv.biases", prefix)),
        ) { cpu_dequant_matvec_4bit(w, s, b, &normed, &mut qkv, qkv_dim, hd, GROUP_SIZE); }
        if let (Some(w), Some(s), Some(b)) = (
            m.wf.get_tensor_u32(&format!("{}.in_proj_z.weight", prefix)),
            m.wf.get_tensor_u16(&format!("{}.in_proj_z.scales", prefix)),
            m.wf.get_tensor_u16(&format!("{}.in_proj_z.biases", prefix)),
        ) { cpu_dequant_matvec_4bit(w, s, b, &normed, &mut z, total_value, hd, GROUP_SIZE); }
        if let (Some(w), Some(s), Some(b)) = (
            m.wf.get_tensor_u32(&format!("{}.in_proj_b.weight", prefix)),
            m.wf.get_tensor_u16(&format!("{}.in_proj_b.scales", prefix)),
            m.wf.get_tensor_u16(&format!("{}.in_proj_b.biases", prefix)),
        ) { cpu_dequant_matvec_4bit(w, s, b, &normed, &mut beta, num_v_heads, hd, GROUP_SIZE); }
        if let (Some(w), Some(s), Some(b)) = (
            m.wf.get_tensor_u32(&format!("{}.in_proj_a.weight", prefix)),
            m.wf.get_tensor_u16(&format!("{}.in_proj_a.scales", prefix)),
            m.wf.get_tensor_u16(&format!("{}.in_proj_a.biases", prefix)),
        ) { cpu_dequant_matvec_4bit(w, s, b, &normed, &mut alpha, num_v_heads, hd, GROUP_SIZE); }

        // 6. Conv1d step
        let mut conv_state = vec![0.0f32; (CONV_KERNEL_SIZE - 1) * qkv_dim];
        let mut conv_out = vec![0.0f32; qkv_dim];
        if let Some(conv_w) = m.wf.get_tensor_u16(&format!("{}.conv1d.weight", prefix)) {
            cpu_conv1d_step(&conv_state, &qkv, conv_w, &mut conv_out, qkv_dim, CONV_KERNEL_SIZE);
        } else {
        ...
        // 8-9. Q/K per-head RMS norm
        let inv_scale = (key_dim as f32).powf(-0.5);
        ...
        for h in 0..num_k_heads {
            ...
            for d in qh_out.iter_mut() { *d *= inv_scale * inv_scale; }
        }
        for h in 0..num_k_heads {
        ...
        // 10. SSM state update
        let a_log = m.wf.get_tensor_f32(&format!("{}.A_log", prefix));
        let dt_bias = m.wf.get_tensor_u16(&format!("{}.dt_bias", prefix));
        let k_heads_per_v = num_v_heads / num_k_heads;
        let mut out_values = vec![0.0f32; total_value];
        let mut ssm_state = vec![0.0f32; num_v_heads * value_dim * key_dim];
        ...
        for vh in 0..num_v_heads {
            ...
            let ssm = &mut ssm_state[s_off..s_off + value_dim * key_dim];
        ...
        if let Some(gnw) = m.wf.get_tensor_u16(&format!("{}.norm.weight", prefix)) {
            for vh in 0..num_v_heads {
        ...
        // 12. Output projection (o_proj)
        let mut attn_out = vec![0.0f32; hd];
        if let (Some(ow), Some(os), Some(ob)) = (
            m.wf.get_tensor_u32(&format!("{}.out_proj.weight", prefix)),
            m.wf.get_tensor_u16(&format!("{}.out_proj.scales", prefix)),
            m.wf.get_tensor_u16(&format!("{}.out_proj.biases", prefix)),
        ) {
            cpu_dequant_matvec_4bit(ow, os, ob, &gated_out, &mut attn_out, hd, total_value, GROUP_SIZE);
        }
        ...
        let to_py = |py: Python<'_>, v: &[f32]| -> PyObject {
            PyArray1::<f32>::from_vec(py, v.to_vec()).into_py(py)
        };
        dict.set_item("normed", to_py(py, &normed))?;
        dict.set_item("qkv", to_py(py, &qkv))?;
        dict.set_item("z", to_py(py, &z))?;
        dict.set_item("beta", to_py(py, &beta))?;
        dict.set_item("alpha", to_py(py, &alpha))?;
        dict.set_item("conv_out", to_py(py, &conv_out))?;
        dict.set_item("lin_q", to_py(py, &lin_q))?;
        dict.set_item("lin_k", to_py(py, &lin_k))?;
        dict.set_item("lin_v", to_py(py, &lin_v))?;
        dict.set_item("q_normed", to_py(py, &q_normed))?;
        dict.set_item("k_normed", to_py(py, &k_normed))?;
        dict.set_item("out_values", to_py(py, &out_values))?;
        dict.set_item("gated_out", to_py(py, &gated_out))?;
        dict.set_item("attn_out", to_py(py, &attn_out))?;
        dict.set_item("hidden_out", to_py(py, &hidden))?;
        Ok(dict.into_py(py))
    }
